...
+(id)inputControlWithDescriptor:(JSResourceDescriptor *)rd tableViewController: (UITableViewController *)tv dataSourceUri: (NSString *)dsUri client:(JSClient *)client
{
	...
	else if ([inputControlType isEqualToString:JS_IC_TYPE_SINGLE_VALUE])  // IC_TYPE_SINGLE_VALUE
	{
		...
		// Check the data type....
		JSResourceDescriptor *datatTypeDescriptor = [JSInputControlCell findDataType:rd];			
...
+(JSResourceDescriptor *)findDataType:(JSResourceDescriptor *)rd
{
	...
	if ([[dataTypeDescriptor wsType] isEqualToString:JS_TYPE_REFERENCE] )
	{
		...
		JSOperationResult *res = [[[JaspersoftAppDelegate sharedInstance] client] resourceInfo:[dataTypeDescriptor resourcePropertyValue:JS_PROP_FILERESOURCE_REFERENCE_URI]];
